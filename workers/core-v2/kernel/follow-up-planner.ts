 ...
 *
 *   · the role must be allowed to ask for that action;
 ...
 *   · the same work twice is the same task — its identity reuses it;
 *   · the target must be a segment of the record, related to the parent's,
 *     and the domain pack says what related, linked and reference mean;
 *   · an independent reader is a new group, never the parent's own;
 *   · a request for human review creates no task at all.
 */
import type {
  AgentActionType, DependencyKind, RequestedAgentAction, SegmentRecord, TaskPhase, TaskRecord, TaskSourceRef,
} from "./contracts.ts";
import { KERNEL_TASK_TYPES } from "./contracts.ts";
import { INDEPENDENCE_GROUPS } from "./domain.ts";
import type { DomainPack } from "./domain.ts";
import { entityId } from "./ids.ts";
import { visibleClaimIdsFor } from "./packet-visibility.ts";
import type { Lookup } from "./planning.ts";
import { CONTRACT_VERSION, hashOfRef, taskIdFor, taskIdentity } from "./planning.ts";
...
const ACTION_TASK: Partial<Record<AgentActionType, (parent: TaskRecord) => { phase: TaskPhase; taskType: string }>> = {
...
export async function planFollowUps(parent: TaskRecord, requests: RequestedAgentAction[], ctx: FollowUpContext): Promise<FollowUpPlan> {
  ...
  for (const request of requests) {
    ...
    if (!request || typeof request !== "object" || !role.allowedActions.includes(request.actionType)) { refuse(`${role.roleKey} may not ask for ${request?.actionType}`); continue; }
    ...
    if (depth > policy.maximumFollowUpDepth) { escalate(`follow-up depth ${depth} exceeds the limit of ${policy.maximumFollowUpDepth}; ${request.actionType} goes to a person`); continue; }
    ...
    if (["open_linked_segment", "expand_segment", "read_related_segment", "read_reference_segment"].includes(request.actionType)) {
      ...
      if (targets.length === 0 || targets.some((t) => t === null)) { refuse("target is not a segment of the record"); continue; }
      const parentSegments = parent.sources.map((s) => (s.segmentId ? lookup.segments.get(s.segmentId) : null)).filter(Boolean) as SegmentRecord[];
      if (!targets.every((t) => pack.isRelated(t!, parent, segments))) { refuse("target segment is not related to what this task was reading"); continue; }
      if (request.actionType === "read_reference_segment" && !targets.every((t) => pack.isReference(t!))) { refuse("read_reference_segment must name reference material"); continue; }
      if (request.actionType === "open_linked_segment" && !targets.every((t) => parentSegments.some((p) => pack.linkedSegments(p, segments).some((l) => l.segmentId === t!.segmentId)))) { refuse("open_linked_segment must name a segment the parent's segments link to"); continue; }
      if (targets.some((t) => t!.status !== "accepted")) { refuse("target segment is not accepted"); continue; }
      ...
      if (sources.length > childRole.maximumSources) { refuse(`${sources.length} sources exceeds ${childRole.roleKey}'s ${childRole.maximumSources}`); continue; }
      ...
      if (!readerTask || readerTask.independenceGroup === null) { refuse("no blind reading to add a reader to"); continue; }
      ...
      if (used.size >= policy.maximumIndependentReadersPerSubject) { escalate(`${readerTask.subjectKey} already has ${used.size} independent readers — a person decides`); continue; }
      ...
      if (seen.has(child.inputFingerprint)) { refuse("the same follow-up was asked for twice in one envelope"); continue; }
      ...
      if (!(await visibleClaimIdsFor(parent, repo, registry, pack)).has(claim.claimId)) { refuse("the claim to check is not one this task produced or was handed"); continue; }
      subjectKey = entityId("subject", claim.claimId);
      targetClaimIds = [claim.claimId];
      sources = [...new Set((await repo.listAnchors([claim.claimId])).map((a) => a.segmentId ?? a.sourceId).filter(Boolean) as string[])]
      ...
      if (dis.followUps.some((f) => f.fingerprint === request.idempotencyFingerprint)) { escalate("the arbiter asked for the same verification again — it goes to a person"); continue; }
...
export function childTask(
  parent: TaskRecord, phase: TaskPhase, taskType: string, subjectKey: string, sources: TaskSourceRef[], independenceGroup: string | null,
  disagreementId: string | null, targetClaimIds: string[], ctx: FollowUpContext, depth: number,
): NewTask {
  ...
  return {
    ...
    maxClaims: role.maximumClaims, dependsOn: [] as { taskId: string; kind: DependencyKind }[],
  };
}
