/* THE ROLES THE KERNEL SHIPS, AND THE REGISTRY A PACK ADDS ITS OWN TO.
 *
 * A role is work: what an agent is handed, what it must return, how much it
 * may read and produce, whether it must be kept blind, what it may ask for.
 * Which executor does it is a routing decision made elsewhere. No role is
 * named after a provider, and the registry's own consistency check refuses
 * one that is.
 */
import type { AgentRoleDefinition, RoleKind } from "./contracts.ts";
import { KERNEL_TASK_TYPES, TASK_PHASES } from "./contracts.ts";
import type { DomainPack } from "./domain.ts";

const CRITIC_ACTIONS = ["expand_segment", "read_reference_segment", "open_linked_segment"] as const;

export const KERNEL_ROLES: AgentRoleDefinition[] = [
  {
...
export const KERNEL_OBJECTIVES: Record<string, string> = {
...
const PACK_PHASES: RoleKind[] = ["analyst", "discoverer", "deriver"];

export class RoleRegistry {
  private roles = new Map<string, AgentRoleDefinition>();
  private byTaskType = new Map<string, AgentRoleDefinition>();
  readonly packId: string;

  constructor(pack: DomainPack) {
    this.packId = pack.id;
    for (const role of [...KERNEL_ROLES, ...pack.roles]) {
      if (this.roles.has(role.roleKey)) throw new Error(`core-v2: role ${role.roleKey} is defined twice`);
      this.roles.set(role.roleKey, role);
      for (const t of role.taskTypes) {
        if (this.byTaskType.has(t)) throw new Error(`core-v2: task type ${t} is served by two roles`);
        this.byTaskType.set(t, role);
      }
    }
  }

  get keys(): string[] { return [...this.roles.keys()]; }
  all(): AgentRoleDefinition[] { return [...this.roles.values()]; }

  role(roleKey: string): AgentRoleDefinition {
    const role = this.roles.get(roleKey);
    if (!role) throw new Error(`core-v2: no role ${roleKey}`);
    return role;
  }

  forTaskType(taskType: string): AgentRoleDefinition {
    const role = this.byTaskType.get(taskType);
    if (!role) throw new Error(`core-v2: no role serves ${taskType}`);
    return role;
  }
  ...
  assertConsistent(): string[] {
    const problems: string[] = [];
    const kernelKeys = new Set(KERNEL_ROLES.map((r) => r.roleKey));
    for (const role of this.roles.values()) {
      if (PROVIDER_WORDS.test(role.roleKey) || PROVIDER_WORDS.test(role.description)) problems.push(`${role.roleKey} names a provider`);
      if (!TASK_PHASES.includes(role.phase)) problems.push(`${role.roleKey} has no phase`);
      if (role.taskTypes.length === 0) problems.push(`${role.roleKey} serves no task type`);
      if (role.maximumClaims < 0 || role.maximumSources < 0 || role.maximumFollowUpDepth < 0) problems.push(`${role.roleKey} has a negative limit`);
      if (role.executorKind === "deterministic" && role.routingProfile !== "deterministic") problems.push(`${role.roleKey} is code but routes to a model`);
      if (role.executorKind === "model" && role.routingProfile === "deterministic") problems.push(`${role.roleKey} is a model but routes to code`);
      if (!kernelKeys.has(role.roleKey)) {
        if (!PACK_PHASES.includes(role.kind)) problems.push(`${role.roleKey} is a pack role of kind ${role.kind} — a pack adds analysts, discoverers and derivers only`);
        for (const t of role.taskTypes) if (!t.startsWith(`${this.packId}:`)) problems.push(`${role.roleKey} serves ${t}, which is not namespaced to ${this.packId}`);
        if (role.kind === "analyst" && role.phase !== "analyze") problems.push(`${role.roleKey} is an analyst outside the analyze phase`);
        if (role.kind === "discoverer" && role.phase !== "discover") problems.push(`${role.roleKey} is a discoverer outside the discover phase`);
        if (role.kind === "deriver" && (role.phase !== "derive" || role.executorKind !== "deterministic")) problems.push(`${role.roleKey} is a deriver that is not deterministic code in the derive phase`);
        if (role.kind === "discoverer" && !role.producesSegments) problems.push(`${role.roleKey} discovers but may not produce segments`);
        if (role.producesAssessments || role.producesAdjudication || role.producesDecisions) problems.push(`${role.roleKey} claims a kernel role's output`);
      }
    }
    for (const key of kernelKeys) if (!this.roles.has(key)) problems.push(`kernel role ${key} is missing`);
    return problems;
  }
}
