 ...
 * not blind. So the policy is applied when the packet is built: an analyst's
 * packet carries no other reader's claims; a critic's packet carries the
 * competing claims under letters and nothing that says who wrote them;
 * nobody's packet carries a provider name, a cost, or a majority count
 * dressed as proof.
 *
 * Visibility is decided by the kind of role, never by its name, so a pack's
 * analysts are as blind as the kernel's would be.
 */
import type { AgentRoleDefinition, Locator, PacketClaim, WorkPacket } from "./contracts.ts";
...
export function authorshipContent(packet: WorkPacket): string[] {
  ...
  for (const c of packet.context.claims) for (const a of c.anchors) { ids.add(a.anchorId); if (a.sourceId) ids.add(a.sourceId); if (a.segmentId) ids.add(a.segmentId); }
  for (const a of packet.context.assessments) for (const id of a.anchorIds) ids.add(id);
  for (const s of packet.sources) { ids.add(s.sourceId); if (s.segmentId) ids.add(s.segmentId); if (s.parentSegmentId) ids.add(s.parentSegmentId); }
  for (const d of packet.context.disagreements) ids.add(d.disagreementId);
  ids.add(packet.workflowId.toLowerCase());
  for (const m of text.matchAll(/[0-9a-f]{8}-[0-9a-f]{4}-5[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/g)) {
    if (!ids.has(m[0])) problems.push(`anonymised packet carries an id that is not an anchor, a place or a disagreement: ${m[0]}`);
  }
  return problems;
}
...
export function assertPacketRespectsVisibility(packet: WorkPacket, role: AgentRoleDefinition): string[] {
  ...
  if (rules.claims === "all_relevant" && role.kind !== "comparator") {
    for (const c of claims) if (c.independenceGroup !== null) problems.push(`${role.roleKey} packet says which reader made ${c.ref}`);
  }
...
export function scrubLocator(locator: Locator): Locator {
  return Object.fromEntries(Object.entries(locator).filter(([k]) => LOCATOR_KEYS.has(k))) as Locator;
}
