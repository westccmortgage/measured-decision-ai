...
import { ENGINE_VERSION, KERNEL_TASK_TYPES } from "./contracts.js";
import { KernelDeterministicExecutor } from "./deterministic.js";
import { INDEPENDENCE_GROUPS } from "./domain.js";
import { childTask, planFollowUps } from "./follow-up-planner.js";
import { canonical, entityId, sha256 } from "./ids.js";
import { PacketOverBudget, buildPacket } from "./packet-builder.js";
import { lookupOf, planCompositions, planDiscovery, segmentIdFor, sourceIdentityOf, specsToTasks, workflowRecordFor } from "./planning.js";
import { budgetOf } from "./policy.js";
import { validateEnvelope } from "./result-validator.js";
import { RoleRegistry } from "./roles.js";
import { StaleState, TERMINAL_TASK_STATES } from "./transitions.js";
export class Scheduler {
    ...
    inFlight = new Map();
    constructor(repo, manifest, pack, policy, router, executors, options) {
        this.repo = repo;
        ...
        if (problems.length)
            throw new Error(`core-v2: the role registry is inconsistent: ${problems.join("; ")}`);
        if (!executors.has("deterministic"))
            executors.register(new KernelDeterministicExecutor(pack), ["deterministic"]);
    ...
    async markNeedsAttention(workflowId) {
        const wf = await this.repo.getWorkflow(workflowId);
        if (!wf || wf.state !== "running")
            return;
        try {
            await this.repo.transitionWorkflow(workflowId, "running", "needs_attention");
        }
        catch (error) {
            if (!(error instanceof StaleState))
                throw error;
        }
    }
    ...
    async plan() {
        ...
        if (workflow.state === "created") {
            if (await this.repo.claimOutbox(wf, this.options.dispatcher ?? this.options.owner))
                workflow = (await this.repo.getWorkflow(wf));
        }
        if (workflow.state === "queued")
            workflow = await this.repo.transitionWorkflow(wf, "queued", "planning");
        ...
        if (refusals.length) {
            if (workflow.state === "planning")
                await this.repo.transitionWorkflow(wf, "planning", "failed", { errorCode: "planning_refused", errorMessage: refusals.join("; ") });
        ...
        if (admission.refused.length) {
            if (workflow.state === "planning")
                await this.repo.transitionWorkflow(wf, "planning", "failed", { errorCode: "budget", errorMessage: admission.refused[0].reason });
    ...
    async expand() {
        ...
        for (const r of refusals)
            await this.repo.audit({ action: "core_v2.expansion.refused", entityType: "intelligence_workflow", entityId: wf, detail: { reason: r } });
    ...
    async cancel(reason) {
        ...
        for (const [attemptId, flight] of this.inFlight)
            this.abort(attemptId, flight.taskId);
        ...
        for (const task of await this.repo.listTasks(wf)) {
            ...
            if (["created", "blocked", "queued", "leased", "running"].includes(task.state)) {
                try {
                    await this.repo.transitionTask(task.taskId, task.state, "cancelled", `cancelled:${reason}`);
                    unsent++;
                }
                catch {
                    submitted++;
            ...
            else if (task.state === "outcome_unknown")
                submitted++;
        }
    ...
    async tick() {
        ...
        for (const t of await this.repo.expireLeases(wf, this.options.now())) {
            ...
            if (t.state === "failed_known" || t.state === "outcome_unknown")
                report.escalations.push(...await this.escalateDisputeOf(t.taskId, t.state));
        }
        ...
        for (const t of active)
            activeByRole.set(t.roleKey, (activeByRole.get(t.roleKey) ?? 0) + 1);
        const timedOut = [...this.inFlight.values()].filter((f) => f.timedOut);
        for (const f of timedOut)
            if (!active.some((t) => t.taskId === f.taskId))
                activeByRole.set(f.roleKey, (activeByRole.get(f.roleKey) ?? 0) + 1);
        ...
        if (timedOut.length >= this.policy.maximumTimedOutExecutions)
            room = 0;
        const runnable = await this.repo.getRunnableTasks(wf);
        const chosen = [];
        if (workflow.state === "needs_attention" && runnable.length)
            workflow = await this.repo.transitionWorkflow(wf, "needs_attention", "running");
        ...
        for (const o of outcomes) {
            ...
            else if (o.state === "outcome_unknown")
            ...
            if (o.state === "failed_known" || o.state === "outcome_unknown")
    ...
    async runUntilQuiescent(maxTicks = this.policy.maximumTasksPerWorkflow + 100) {
        let ticks = 0;
        for (; ticks < maxTicks; ticks++) {
            const r = await this.tick();
            if (r.dispatched.length === 0 && r.released === 0 && r.stopped === 0 && r.reconciled === 0 && this.inFlight.size === 0)
                break;
        }
        if (ticks >= maxTicks)
            throw new Error(`core-v2: still working after ${maxTicks} ticks — that is a loop, not a workflow`);
    ...
    async advanceWorkflow() {
        ...
        const restDone = await (async () => { for (const t of rest)
            if (open(t) && !(await blockedByPerson(t)))
        ...
        if ((workflow.state === "running" || workflow.state === "needs_attention") && restDone && this.inFlight.size === 0) {
            ...
            for (const r of admitted.refused)
                this.escalations.push(`compose ${r.task.subjectKey}: ${r.reason}`);
    ...
    async settleWorkflow() {
        const wf = this.manifest.workflowId;
        const workflow = (await this.repo.getWorkflow(wf));
        if (workflow.cancelRequestedAt !== null || ["completed", "partial", "failed", "cancelled"].includes(workflow.state))
            return;
        const tasks = await this.repo.listTasks(wf);
        ...
        if (workflow.state === "running" && tasks.some((t) => t.state === "blocked"))
            await this.repo.transitionWorkflow(wf, "running", "needs_attention");
    }
    /* ───────────────────────────────────────────────── one assignment */
    async runTask(task) {
        const done = (state, extra = {}) => ({ taskId: task.taskId, state, childrenCreated: 0, childrenReused: 0, escalations: [], discovered: false, ...extra });
        ...
        const failBeforeAttempt = async (reason, code, message, family = "none", domain = "none") => {
        ...
        try {
            ...
            return failBeforeAttempt(code, code, String(error.message));
        }
        ...
        const unsent = async (reason) => {
            ...
            if (now && (now.state === "leased" || now.state === "running")) {
                try {
                    await this.repo.transitionTask(task.taskId, now.state, "cancelled", reason);
                }
                catch { /* reconciled elsewhere */ }
            }
        ...
        try {
            ...
            return unsent("task_no_longer_leased");
        }
        ...
        if (!submitted.ok)
            return unsent(`submission refused: ${submitted.reason}`);
        ...
        try {
            ...
            try {
                await this.repo.commitValidatedResult(this.emptyCommit(task, attempt.attemptId, null, "outcome_unknown", "outcome_unknown", `${code}: ${error.message}`, [String(error.message)], facts, code));
            }
            catch {
                try {
                    await this.repo.transitionAttempt(attempt.attemptId, "submitted", "outcome_unknown", { errorCode: code, errorMessage: String(error.message) });
                }
                catch { /* already moved */ }
                const now = await this.repo.getTask(task.taskId);
        ...
        try {
            ...
            if (current && current.state === "running") {
                ...
                if (a && !["succeeded", "failed_known", "outcome_unknown", "output_limited"].includes(a.state)) {
                    try {
                        await this.repo.commitValidatedResult(this.emptyCommit(task, attempt.attemptId, raw, "failed_known", "failed_known", `engine_error: ${error.message}`, [`engine: ${error.message}`], facts));
                    }
                    catch {
                        try {
                            await this.repo.transitionTask(task.taskId, "running", "failed_known", `engine_error: ${error.message}`);
                        }
                ...
                await this.repo.audit({ action: "core_v2.task.engine_error", entityType: "workflow_task", entityId: task.taskId, detail: { message: String(error.message) } });
            }
            return done("failed_known");
        }
    }
    ...
    emptyCommit(task, attemptId, raw, attemptTo, taskTo, reason, problems, providerFacts = {}, taskReason = reason) {
        return {
    ...
    startHeartbeat(taskId, leaseToken) {
    ...
    async reconcileUnknown(workflowId) {
        let reconciled = 0;
        for (const task of (await this.repo.listTasks(workflowId)).filter((t) => t.state === "outcome_unknown")) {
            for (const attempt of (await this.repo.listAttempts(task.taskId)).filter((a) => a.state === "outcome_unknown" && a.reconciliationOutcome === null)) {
                if (this.inFlight.has(attempt.attemptId))
                    continue;
    ...
    async usedDomainsFor(task) {
        const role = this.registry.role(task.roleKey);
        if (task.independenceGroup)
            return this.repo.independenceDomainsForSubject(task.workflowId, task.subjectKey);
    ...
    async agreeingReadings(claim) {
        if (!claim.independenceGroup)
            return [];
    ...
    async domainsBehind(claimIds) {
        ...
        for (const id of claimIds) {
            ...
            if (claim.independenceDomain)
                domains.add(claim.independenceDomain);
            for (const peer of await this.agreeingReadings(claim))
                if (peer.independenceDomain)
                    domains.add(peer.independenceDomain);
        }
        return [...domains];
    }
    newAttempt(task, attemptId, attemptNo, kind, family, domain, configuration, packetFingerprint, packetBytes, leaseToken) {
        return {
    ...
    /* ─────────────────────────────────────────────── settling an answer */
    async settle(task, attempt, packet, refToClaimId, envelope, raw, facts, lookup, done) {
        const role = this.registry.role(task.roleKey);
        ...
        let validation;
        try {
            validation = validateEnvelope(packet, envelope, { role, pack: this.pack, lookup, policy: this.policy });
        }
        catch (error) {
            validation = { ok: false, problems: [`the envelope could not be validated: ${error.message}`] };
        }
        if (!validation.ok) {
        ...
        for (const r of outcome.children.refused) {
            ...
            if (r.task.disagreementId)
                escalations.push(...await this.escalateDisagreement(r.task.disagreementId, task, r.reason));
        }
        for (const f of outcome.followUpsRefused)
            escalations.push(...await this.escalateDisagreement(f.disagreementId, task, f.reason));
        for (const child of [...outcome.children.created, ...outcome.children.reused]) {
            const kind = s.rewire.find((r) => r.child === child.taskId)?.kind;
            if (kind)
                await this.rewireDependents(task, child.taskId, kind);
        }
    ...
    async settlementOf(task, attempt, packet, refToClaimId, envelope, lookup) {
        const s = { segments: [], claims: [], assessments: [], disagreements: [], claimTransitions: [], disagreementRounds: [], disagreementTransitions: [], decisions: [], children: [], dependencies: [], followUps: [], taskTransitions: [], audits: [], escalations: [], rewire: [] };
        ...
        if (task.phase === "ingest") {
            ...
            if (source)
                for (const d of source.declaredSegments)
                    s.segments.push(this.newSegment(wf, d, "deterministic", attempt.attemptId, lookup));
        }
        for (const p of envelope.segments)
            s.segments.push(this.newSegment(wf, p, this.registry.role(task.roleKey).executorKind === "model" ? "model" : "deterministic", attempt.attemptId, lookup));
        for (const seg of s.segments)
            lookup.segments.set(seg.segmentId, { ...seg });
        /* Claims and their anchors. */
        const anchorByKey = new Map(envelope.anchors.map((a) => [a.anchorKey, a]));
        const claimIdOf = new Map();
        for (const c of envelope.claims) {
            const claimId = entityId("claim", attempt.attemptId, c.claimKey);
            claimIdOf.set(c.claimKey, claimId);
            const anchors = [...new Set(c.anchorKeys)].map((key) => {
        ...
        if (requests.length && disputeHeld) {
            ...
            for (const e of plan.escalations)
                s.escalations.push(`${task.taskId}: ${e.reason}`);
            for (const r of plan.refused)
                s.audits.push({ action: "core_v2.follow_up.refused", entityType: "workflow_task", entityId: task.taskId, detail: { action: r.request.actionType, reason: r.reason } });
            for (const e of plan.escalations)
                s.audits.push({ action: "core_v2.follow_up.escalated", entityType: "workflow_task", entityId: task.taskId, detail: { action: e.request.actionType, reason: e.reason } });
        }
        return s;
    }
    newSegment(wf, d, by, attemptId, lookup) {
        ...
        const status = parent || d.parentSegmentId === null ? "accepted" : "proposed";
    ...
    async rewireDependents(parent, childTaskId, kind) {
        for (const d of await this.repo.getDependents(parent.taskId)) {
            ...
            if (!dependent || !["created", "blocked", "queued"].includes(dependent.state))
                continue;
            if (dependent.taskId === childTaskId)
                continue;
            try {
                await this.repo.addDependency(dependent.taskId, childTaskId, kind, this.limits);
            }
            catch (error) {
                this.escalations.push(`${dependent.taskId}: ${error.message}`);
            }
        }
    }
    /* ───────────────────────────────────────── what a result makes necessary */
    async afterCompletion(task, attempt, envelope, s, refToClaimId, lookup) {
        const role = this.registry.role(task.roleKey);
        switch (task.phase) {
            case "ingest":
                ...
                for (const c of s.claims) {
                    ...
                    if (c.predicate === "content_identity" && identity && c.value.text === identity && c.anchors.length)
                        this.acceptByRule(task, attempt, c.claimId, "source_identity_matches_manifest", [], s);
                }
                return;
            case "analyze":
            case "discover":
            ...
            case "derive":
                ...
                for (const c of s.claims) {
                    ...
                    if (inputs.length && inputs.every((i) => i && i.status === "accepted") && c.anchors.length)
                        this.acceptByRule(task, attempt, c.claimId, `derived:${task.taskType}`, c.inputClaimIds, s);
                }
                return;
            case "compose":
                for (const d of envelope.decisions) {
                    s.decisions.push({ decideTo: null, decision: {
                            ...
                            evidence: [
                                ...d.supportingClaimIds.map((id) => ({ claimId: refToClaimId[id] ?? id, anchorId: null, link: "supports", rule: null })),
                                ...d.contradictingClaimIds.map((id) => ({ claimId: refToClaimId[id] ?? id, anchorId: null, link: "contradicts", rule: null })),
                            ],
    ...
    async afterComparison(task, envelope, s, refToClaimId, lookup) {
        const toVerify = [];
        for (const group of envelope.calculations) {
            const members = (await Promise.all(group.inputClaimIds.map((id) => this.repo.getClaim(refToClaimId[id] ?? id)))).filter(Boolean);
            if (new Set(members.map((m) => m.independenceGroup)).size < 2)
                continue;
            for (const m of members)
                if (m.status === "proposed")
                    s.claimTransitions.push({ claimId: m.claimId, from: "proposed", to: "corroborated" });
            ...
            if (representative.anchorIds.length && !representative.incompleteSourceAttempt && representative.value.known)
                toVerify.push(representative.claimId);
        }
        if (toVerify.length) {
            const anchors = await this.repo.listAnchors(toVerify);
            const places = [...new Set(anchors.map((a) => a.segmentId ?? a.sourceId).filter(Boolean))];
        ...
        for (const dis of toDispute) {
            if (this.policy.escalateImmediatelyAtSeverity && dis.severity === this.policy.escalateImmediatelyAtSeverity) {
                this.escalateNew(dis, task, "severity policy sends this straight to a person", s);
                continue;
            }
    ...
    async afterCriticism(task, attempt, s, refToClaimId, lookup) {
        const disputeKindOf = { contradicts: "value", wrong_unit: "unit", wrong_scope: "scope", duplicate: "duplicate" };
        for (const a of s.assessments) {
            const claim = await this.repo.getClaim(a.claimId);
            if (!claim || !["proposed", "corroborated"].includes(claim.status))
                continue;
            ...
            if ((await this.domainsBehind([claim.claimId])).includes(attempt.independenceDomain))
                continue;
            if (a.assessment === "supports") {
                if (claim.anchorIds.length === 0 || claim.incompleteSourceAttempt)
                    continue;
            ...
            if (s.disagreements.some((d) => d.disagreementKey === key) || (await this.repo.listDisagreements(task.workflowId)).some((d) => d.disagreementKey === key))
                continue;
            const kind = disputeKindOf[a.assessment];
            const dis = { disagreementId: entityId("disagreement", task.workflowId, key), workflowId: task.workflowId, disagreementKey: key, kind: kind ?? "coverage", severity: "material", subjectSignature: { ...signature, subject_key: claim.subjectKey, found_by: "verification" }, claimIds: members.map((m) => m.claimId).sort() };
            s.disagreements.push(dis);
            if (!kind) {
                this.escalateNew(dis, task, `the reopened source could not be read for this claim (${a.assessment}); a person decides`, s);
                continue;
            }
            await this.disputeChildren(task, dis, s, lookup, refToClaimId);
        }
    ...
    async disputeChildren(task, dis, s, lookup, _refs) {
        const anchors = await this.repo.listAnchors(dis.claimIds);
        const places = [...new Set(anchors.map((a) => a.segmentId ?? a.sourceId).filter(Boolean))];
        const ctx = this.followUpContext(lookup);
        const toRefs = (n) => places.slice(0, n).map((id) => (lookup.segments.has(id) ? { sourceId: null, segmentId: id } : { sourceId: id, segmentId: null }));
    ...
    async afterVerification(task, attempt, envelope, s, lookup) {
        if (!task.disagreementId)
            return;
        const dis = await this.repo.getDisagreement(task.disagreementId);
        if (!dis || dis.state === "needs_human" || dis.state === "resolved")
            return;
        ...
        if (envelope.outcome === "insufficient_evidence") {
            ...
            envelope.requestedActions = [];
            const plan = await planFollowUps(task, requests, this.followUpContext(lookup));
            if (plan.escalations.length || plan.children.length === 0) {
                this.escalateExisting(dis, task, plan.escalations[0]?.reason ?? plan.refused[0]?.reason ?? "no follow-up could be made", s);
                return;
            }
            ...
            for (const arb of pendingArbiters)
                s.dependencies.push({ taskId: arb.taskId, dependsOnTaskId: child.taskId, kind: "requires_completion" });
        }
    }
    async afterAdjudication(task, attempt, envelope, s, refToClaimId, lookup) {
        const proposal = envelope.adjudication;
        if (!proposal || !task.disagreementId)
            return;
        const dis = await this.repo.getDisagreement(task.disagreementId);
        if (!dis || dis.state === "resolved" || dis.state === "needs_human")
            return;
        ...
        switch (proposal.outcome) {
            case "accept_claim": {
                ...
                if (!acceptedId)
                    return this.escalateExisting(dis, task, "the arbiter accepted a claim the packet did not present", s);
            ...
            case "correct": {
                ...
                if (!template || anchorsOf.length === 0)
                    return this.escalateExisting(dis, task, "a correction needs source anchors", s);
                ...
                if (anchors.length === 0)
                    return this.escalateExisting(dis, task, "the correction rests on anchors the record does not hold", s);
            ...
            case "needs_more_evidence": {
                ...
                if (dis.followUps.some((f) => f.fingerprint === request.idempotencyFingerprint))
                    return this.escalateExisting(dis, task, "the arbiter asked for the same evidence again — a person decides", s);
                if (rounds >= this.policy.maximumArbiterRounds)
                    return this.escalateExisting(dis, task, `${rounds} rounds of arbitration asked for more evidence`, s);
                if (dis.followUps.some((f) => f.round === dis.criticRounds + 1))
                    return this.escalateExisting(dis, task, "this round already has its one follow-up — a person decides", s);
                const plan = await planFollowUps(task, [request], this.followUpContext(lookup));
                if (plan.children.length === 0)
                    return this.escalateExisting(dis, task, plan.escalations[0]?.reason ?? plan.refused[0]?.reason ?? "no follow-up could be made", s);
    ...
    async resolveWith(task, attempt, dis, type, proposal, acceptedClaimId, anchorIds, s, label = type, reviewedElsewhere = false) {
        ...
        if (acceptedClaimId) {
            ...
            const anchorsCount = pending ? pending.anchors.length : accepted?.anchorIds.length ?? 0;
            if (!accepted || anchorsCount === 0 || accepted.incompleteSourceAttempt)
                return this.escalateExisting(dis, task, "the arbiter accepted a reading that is unanchored or was cut short — a person decides", s);
            if (!await this.acceptanceCanStand(task.workflowId, acceptedClaimId, accepted.independenceDomain ?? null, reviewedElsewhere)) {
                return this.escalateExisting(dis, task, "nothing from outside the domain that produced this value has confirmed it: an acceptance needs a reading of the source from another domain, a deterministic rule or a person, so a person decides", s);
            }
        ...
        for (const id of dis.claimIds) {
            if (id === acceptedClaimId)
                continue;
            const claim = await this.repo.getClaim(id);
            if (claim && claim.status === "accepted")
                s.claimTransitions.push({ claimId: id, from: "accepted", to: "superseded" });
            else if (claim && claim.status !== "rejected")
                s.claimTransitions.push({ claimId: id, from: claim.status, to: "rejected" });
            evidence.push({ claimId: id, anchorId: null, link: "contradicts", rule: null });
        }
        for (const a of anchorIds)
            evidence.push({ claimId: null, anchorId: a, link: "context", rule: null });
    ...
    async acceptanceCanStand(workflowId, claimId, domain, reviewedElsewhere) {
        if (!domain)
            return true;
    ...
    escalateNew(dis, task, reason, s) {
        ...
        s.decisions.push({ decideTo: "needs_human", decision: {
                ...
                evidence: dis.claimIds.map((id) => ({ claimId: id, anchorId: null, link: "context", rule: null })), actions: [{ actionType: "review", ownerRole: "reviewer" }], summary: null,
            } });
    ...
    async escalateExisting(dis, task, reason, s) {
        if (s.disagreementTransitions.some((t) => t.disagreementId === dis.disagreementId))
            return;
        for (const id of dis.claimIds) {
            const c = await this.repo.getClaim(id);
            if (c && c.status === "disputed" && !s.claimTransitions.some((t) => t.claimId === id))
                s.claimTransitions.push({ claimId: id, from: "disputed", to: "unresolved" });
        }
        const decisionId = entityId("decision", dis.disagreementId, "needs_human");
        ...
        for (const t of await this.repo.listTasks(task.workflowId)) {
            if (t.disagreementId === dis.disagreementId && t.taskId !== task.taskId && ["created", "blocked", "queued"].includes(t.state)) {
                if (t.state === "created")
                    s.taskTransitions.push({ taskId: t.taskId, from: "created", to: "blocked", reason: null });
                s.taskTransitions.push({ taskId: t.taskId, from: t.state === "created" ? "blocked" : t.state, to: "superseded", reason: "needs_human" });
            }
        }
        s.escalations.push(`${dis.subjectSignature.subject_key ?? dis.disagreementKey}: ${reason}`);
        s.audits.push({ action: "core_v2.disagreement.needs_human", entityType: "disagreement", entityId: dis.disagreementId, detail: { reason } });
    ...
    async escalateDisagreement(disagreementId, task, reason) {
        const dis = await this.repo.getDisagreement(disagreementId);
        if (!dis || (dis.state !== "open" && dis.state !== "verifying"))
            return [];
        const decisionId = entityId("decision", dis.disagreementId, "needs_human");
        for (const id of dis.claimIds) {
            const c = await this.repo.getClaim(id);
            if (c && c.status === "disputed")
                await this.repo.transitionClaim(id, "disputed", "unresolved");
        }
        if (!(await this.repo.getDecision(decisionId))) {
        ...
        for (const t of await this.repo.listTasks(task.workflowId)) {
            if (t.disagreementId === dis.disagreementId && t.taskId !== task.taskId && ["created", "blocked", "queued"].includes(t.state)) {
                if (t.state === "created")
                    await this.repo.transitionTask(t.taskId, "created", "blocked");
    ...
    async escalateDisputeOf(taskId, state) {
        ...
        if (task.independenceGroup)
            return this.holdSubjectForCoverage(task, reason);
        return [];
    }
    ...
    async holdSubjectForCoverage(task, reason) {
        ...
        if (claimIds.length === 0)
            claimIds.push(...task.targetClaimIds.filter((id) => others.some((c) => c.claimId === id) || true));
    ...
    acceptByRule(task, attempt, claimId, rule, corroboratingClaimIds, s, anchorIds = [], from = "proposed") {
        if (!s.claimTransitions.some((t) => t.claimId === claimId && t.to === "accepted"))
            s.claimTransitions.push({ claimId, from, to: "accepted" });
        s.decisions.push({ decideTo: "machine_decided", decision: {
                ...
                evidence: [
                    ...
                    ...corroboratingClaimIds.map((id) => ({ claimId: id, anchorId: null, link: "context", rule })),
                    ...anchorIds.map((id) => ({ claimId: null, anchorId: id, link: "context", rule })),
                ],
...
export function normaliseEnvelope(raw) {
    ...
    return {
        ...
        outcome: isEnvelope ? r.outcome : "failed_known",
        ...
        limitations: isEnvelope ? arr(r.limitations) : [`the executor returned something that is not an envelope: ${JSON.stringify(raw)?.slice(0, 200) ?? String(raw)}`],
