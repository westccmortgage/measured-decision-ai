...
import { canonical, entityId, fingerprint, sha256 } from "./ids.js";
...
export function taskIdentity(workflowId, phase, taskType, subjectKey, sourceHashes, independenceGroup) {
    return fingerprint({ workflowId, phase, taskType, subjectKey, sourceHashes, independenceGroup, contract: CONTRACT_VERSION });
}
export function taskIdFor(workflowId, identity) {
...
export function segmentIdFor(workflowId, sourceId, parentSegmentId, segmentKind, contentHash) {
    return entityId("segment", workflowId, sourceId, parentSegmentId ?? "", segmentKind, contentHash);
}
export function sourceIdentityOf(source) {
    if (source.contentHash)
        return `digest=${source.hashAlgorithm ?? "sha256"}:${source.contentHash}`;
    if (source.objectVersionId)
        return `version=${source.objectVersionId}`;
    throw new Error(`core-v2: source ${source.sourceId} has neither a content hash nor a version — it is not read`);
}
export function lookupOf(manifest, segments) {
    return {
        sources: new Map(manifest.sources.map((s) => [s.sourceId, s])),
        segments: new Map(segments.map((s) => [s.segmentId, s])),
    };
}
export function hashOfRef(ref, lookup) {
    if (ref.segmentId) {
...
export function specsToTasks(specs, workflowId, lookup, registry, policy, parent = null, depth = 0) {
    const refusals = [];
    ...
    for (const spec of specs) {
        ...
        if (role.roleKey !== spec.roleKey) {
            refusals.push(`${spec.key}: ${spec.taskType} is ${role.roleKey}'s work, not ${spec.roleKey}'s`);
            continue;
        }
        if (role.phase !== spec.phase) {
            refusals.push(`${spec.key}: ${spec.taskType} belongs to the ${role.phase} phase`);
            continue;
        }
        if (spec.sources.length > role.maximumSources) {
            refusals.push(`${spec.key}: ${spec.sources.length} sources exceeds ${role.roleKey}'s ${role.maximumSources}`);
            continue;
        }
        if (spec.sources.length > policy.maximumPacketSources) {
            refusals.push(`${spec.key}: ${spec.sources.length} sources exceeds the policy's ${policy.maximumPacketSources}`);
            continue;
        }
        ...
        if (allSourceIds.size > 1 && wholeSourcesNamed.size >= allSourceIds.size && spec.phase !== "compose") {
            refusals.push(`${spec.key}: one assignment over every source of the workflow, whole`);
            continue;
        }
        if (spec.independenceGroup && !role.requiresIndependentReading) {
            refusals.push(`${spec.key}: ${role.roleKey} is not read blind, yet the spec names a group`);
            continue;
        }
        if (idByKey.has(spec.key)) {
            refusals.push(`${spec.key}: two specs share one key`);
            continue;
        }
        let hashes;
        try {
            hashes = spec.sources.map((s) => hashOfRef(s, lookup));
        }
        catch (error) {
            refusals.push(`${spec.key}: ${error.message}`);
            continue;
        }
    ...
    for (const { spec, id, identity } of drafts) {
        ...
        for (const d of spec.dependsOn) {
            ...
            if (!target) {
                refusals.push(`${spec.key}: depends on ${d.key}, which this expansion does not define`);
                broken = true;
                break;
            }
            if (target === id) {
                refusals.push(`${spec.key}: depends on itself`);
                broken = true;
                break;
            }
        ...
        if (broken)
            continue;
        for (const d of spec.dependsOnTaskIds)
            dependsOn.push({ taskId: d.taskId, kind: d.kind });
        tasks.push({
...
export function workflowRecordFor(manifest, pack, budget) {
...
export function planDiscovery(manifest, pack) {
    const specs = [];
    for (const source of manifest.sources) {
        ...
        if (discoverer) {
            ...
            if (!role || role.kind !== "discoverer" || role.taskTypes.length === 0)
                throw new Error(`core-v2: ${pack.id} names ${discoverer} to discover ${source.sourceKind}, which is not one of its discoverer roles`);
            specs.push({
...
export function planCompositions(subjects, dependsOnTaskIds) {
    return [...new Set(subjects)].sort().map((subject) => ({
        ...
        dependsOnTaskIds: dependsOnTaskIds.map((taskId) => ({ taskId, kind: "requires_completion" })),
    }));
}
/* A short, readable account of a graph, for the dry run. */
export function describeTasks(tasks, lookup) {
    const lines = [];
    const byRole = {};
    for (const t of tasks)
        byRole[t.roleKey] = (byRole[t.roleKey] ?? 0) + 1;
    lines.push(`${tasks.length} bounded assignments`);
    lines.push("");
    lines.push("by role:");
    for (const [role, n] of Object.entries(byRole).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])))
        lines.push(`  ${String(n).padStart(4)}  ${role}`);
    ...
    if (blind.length) {
        ...
        for (const t of blind)
            bySubject.set(`${t.taskType} ${t.subjectKey}`, [...(bySubject.get(`${t.taskType} ${t.subjectKey}`) ?? []), t.independenceGroup]);
        for (const [k, groups] of bySubject)
            lines.push(`  ${k}  → ${groups.sort().join(" | ")}`);
    }
    lines.push("");
    lines.push("tasks:");
    for (const t of tasks) {
