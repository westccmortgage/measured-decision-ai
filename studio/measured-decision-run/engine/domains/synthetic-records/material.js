 ...
 */
import { deflateSync, inflateSync } from "../../../engine-shims/node-zlib.js";
...
export function renderTable(region) {
    const lines = [
        `# ${region.label}`,
...
export function renderNoteText(note, tableHash) {
...
export function renderSource(label, sheets) {
    const lines = [`# ${label}`, "sheet  ordinal  content-hash"];
    for (const s of sheets)
        lines.push(`${s.label}  ${s.ordinal}  ${s.contentHash}`);
    return `${lines.join("\n")}\n`;
}
/* ───────────────────────────────────────────────────────────── the image */
const ROW_WIDTH = 16;
/* Text as an 8-bit greyscale PNG, sixteen bytes to a row. */
export function renderTextAsImage(text) {
...
export function readTextFromImage(bytes) {
    const buffer = Buffer.from(bytes);
    const signature = Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);
    if (buffer.length < 8 || !buffer.subarray(0, 8).equals(signature))
        throw new Error("synthetic-records: that is not a PNG");
    let at = 8;
    let width = 0;
    let height = 0;
    const parts = [];
    ...
    if (width === 0 || height === 0)
        throw new Error("synthetic-records: the image says it has no size");
    const raw = inflateSync(Buffer.concat(parts));
    const pixels = Buffer.alloc(width * height);
    for (let y = 0; y < height; y++) {
        if (raw[y * (width + 1)] !== 0)
            throw new Error("synthetic-records: this reader only understands unfiltered rows");
