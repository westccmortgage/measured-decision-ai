...
const MODELS = {
  openai: { id: "gpt-5.6-sol", input: 4, output: 20, key: "OPENAI_API_KEY" },
  anthropic: { id: "claude-opus-5", input: 5, output: 25, key: "ANTHROPIC_API_KEY" },
  google: { id: "gemini-3.1-pro", input: 2, output: 12, key: "GEMINI_API_KEY" },
};
/* Rough per-image token estimate used only for the pre-flight budget check. */
const IMAGE_TOKENS = { openai: 5000, anthropic: 4784, google: 5200 };
const PDF_PAGE_TOKENS = 3000;
const OUTPUT_TOKENS = 15000;
...
for (const p of providers) {
  ...
  const usd = (inTok * MODELS[p].input + OUTPUT_TOKENS * MODELS[p].output) / 1e6;
  estimate[p] = { input_tokens: inTok, output_tokens: OUTPUT_TOKENS, usd: +usd.toFixed(2) };
  total += usd;
}
console.log("Estimate:", JSON.stringify(estimate), `total ≈ $${total.toFixed(2)} (cap $${budget})`);
if (total > budget) { console.error("Refusing: estimate exceeds the approved budget."); process.exit(3); }
...
const requests = {
  openai: () => ({
    ...
    body: {
      model: MODELS.openai.id, store: false,
  ...
  anthropic: () => ({
    ...
    body: {
      model: MODELS.anthropic.id, max_tokens: 32000, thinking: { type: "adaptive" }, output_config: { effort: "high" },
...
const spent = ledger.reduce((s, r) => s + (r.cost_usd || 0), 0);
