export const AGENT_CONTRACT_VERSION = "2026-09-07.1";
...
export const PLAN_WORKFLOW_INSTRUCTIONS = [
  ...
  "- A legend of countable devices is scope too. An ELECTRICAL SYMBOL LEGEND — Wall Switch S, 3-way S3, S3D with dimmer, SD dimmer, SOS occupancy sensor, duplex and GFCI receptacles, smoke and CO detectors, exhaust fans, data, TV and telephone outlets, doorbells, thermostats — prints no quantity column, yet every entry names a device the plans place by symbol. Record one component_schedule row per legend entry with category electrical_device: mark is the legend's tag as printed (S3D; the legend's own label when it prints no tag), description verbatim. count_scheduled is 0 — a legend prints no quantity — and count_drawn is the number of that symbol you counted on every floor plan, reflected ceiling plan and electrical plan, tile by tile, citing each sheet and level in count_note. When the symbols cannot be counted with certainty, propose with count_confidence and count_note. When the legend defines a symbol and you found none drawn, the row still exists with all counts 0 and a note saying so — a person needs to see what the set defines, not only what it counts. Naming switches in a system's scope sentence and recording no rows is the failure this rule refuses.",
  ...
  "- Record a structural_member for each row of every printed BEAM SCHEDULE, HEADER SCHEDULE, FLOOR/CEILING JOIST SCHEDULE, RAFTER SCHEDULE, POST/COLUMN SCHEDULE, FOOTING/FOUNDATION SCHEDULE, HOLD-DOWN and ANCHOR SCHEDULE, SHEAR WALL SCHEDULE, and for each member a framing or foundation plan labels by mark (FB1, HDR2, RB 1, RIDGE BM 1, F.J.1, F1 footing, HDU4, SSW 15x8). A structural set states its countable scope in these schedules the way an architectural set states it in door and window schedules. Copy the mark and the description verbatim; size, spacing and material as printed; member_type from the schedule the row lives in. count_scheduled is a quantity the schedule prints; count_drawn is the number of that mark you counted drawn on the plans, tile by tile, citing the sheet; when certainty is out of reach, propose in count_proposed with count_confidence and count_note, exactly as with framing_deck members. A member drawn and labelled on a plan without a schedule row is still a member: its callout is its description and the missing schedule row is a gap. length_printed holds a printed length or stays empty — nothing here is measured by scale. Cite the sheet for every member and the detail it points to in detail_refs.",
  "- What the number counts. Every structural_member count says what it counted in counted: members — discrete pieces, one label one member (FB1 ×8); assemblies — a built-up member the schedule prints as plies, \"(2)-2x10\" or \"(3) 2x12\": count the assemblies, put the ply count in plies, and never multiply; zones — a framing zone the plan marks with a spacing and an arrow (R.R.1 2x10 @16'' O.C., F.J.1, D.J.1): count the zones, never the pieces, because the number of rafters or joists in a zone follows from its dimensions and spacing and this reading measures nothing by scale; labels — callouts seen whose relation to pieces is not certain; none — nothing was counted. A count of zones or labels is never a count of members: twelve R.R.1 zones are not twelve rafters, and saying so with low confidence does not repair saying the wrong thing.",
  "- A mark on the plan reaches its size by a path a person can check, named in size_basis: its own schedule row (schedule_row); a printed rule the sheet states — a header table by opening width, \"4x12 MIN. HEADER U.N.O.\" — quoted in count_note (printed_rule); only the plan's callout (plan_label); or not at all (not_resolved). An unnumbered label never resolves to the first numbered row of a schedule: RHDR is not RHDR 1. When the sheets print a rule that assigns the size, cite the rule; when they do not, the label is counted as labels with size_basis not_resolved and the size left empty, and the question names the label and the rows it might be.",
  "- Every printed schedule row is a structural_member row, whether or not an instance was found drawn: HDR4 printed in the header schedule with no HDR4 callout found on the plans read is still recorded — counts 0, counted none, count_note \"schedule row printed; no instance found on the plans read\". A person needs to see what the set defines, not only what was counted. Dropping HDR4 because it was not seen on a plan is the failure this rule refuses.",
  "- framing_defaults holds the printed general rules the sheets state for members not individually scheduled: \"ALL STUDS 2x4 #2 @ 16'' O.C. U.N.O.\", a typical header rule, a nailing rule. Copy the rule verbatim in rule, say what kind of rule it is in kind (studs, plates, sheathing, blocking, nailing, headers, joists, rafters, connectors, lumber, other), name what it governs in applies_to, and copy its exception clause verbatim in exception (\"unless noted otherwise\", \"U.N.O.\", \"see plan\"). A printed rule is a project requirement with its exception — never an assumption of ours. Where a member's size follows only from such a rule, say so in that member's count_note.",
